...
def create_gps_column(df, colonne_latitude, colonne_longitude, colonne_profondeur):
    ...
    gps_info_list = [] # Liste pour stocker les informations GPS
    gps_dict_list = []
...
def import_gps_from_xlsx(file_path, nom_colonne, prefixe, colonne_latitude, colonne_longitude, colonne_profondeur):

    df = pd.read_excel(file_path)
    filtered_df = df[df[nom_colonne].str.contains(prefixe, na=False)].copy()

    result_df = filtered_df[[nom_colonne, colonne_latitude, colonne_longitude, colonne_profondeur]].copy()  # Créer une copie explicite
...
def add_gps_to_photos(df, photos_directory, nom_colonne, identifiant_xlsx, identifiant_photo):
    ...
    new_directory = os.path.join(photos_directory, 'NEW') # Créer le sous-dossier 'NEW' s'il n'existe pas déjà
    ...
    for index, row in df.iterrows():     # Parcourir chaque ligne du dataframe
        ...
        for filename in os.listdir(photos_directory):
            if f"_{identifiant_photo}{station_number}_" in filename: # Si le nom du fichier de l'image contient _P{i} (par exemple _P1_ pour Pilier 1)
...
source_file_path_xlsx = 'C:/Users/ALB/Desktop/TEMP/PILIER/OFB2023_Pilier_stations_pour_metadonnées.xlsx' #Fichier xlsx issu de QGIS
...
new_file_path_xlsx = 'C:/Users/ALB/Desktop/TEMP/TEST/Nouveau_fichier_avec_photos.xlsx' #Nom du nouveau fichier Excel avec les photos
...
photos_directory = 'C:/Users/ALB/Desktop/TEMP/TEST'  # Remplacez par le chemin du répertoire des photos
